...
main( int argc, char *argv[ ] )
{
	...
	for( int k = 0; k < NUMCAPITALS; k++ )
	{
		...
		Capitals[k].name = Cities[closest_capital].name;
		// Capitals[k].longitude = Cities[closest_capital].longitude;
		// Capitals[k].latitude = Cities[closest_capital].latitude;
	}
...
#ifdef CSV
        printf("%2d , %4ld , %4d , %8.3lf\n", NUMT, NUMCITIES, NUMCAPITALS, megaCityCapitalsPerSecond );
#else
        printf("%2d threads : %4ld cities ; %4d capitals; megatrials/sec = %8.3lf\n",
                NUMT, NUMCITIES, NUMCAPITALS, megaCityCapitalsPerSecond );
#endif
