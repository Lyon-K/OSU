...
main( int argc, char *argv[ ] )
{
	...
	status = clEnqueueWriteBuffer( CmdQueue, dX, CL_FALSE, 0, DATASIZE, hX, 0, NULL, NULL );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clEnqueueWriteBuffer failed (1)\n" );

	status = clEnqueueWriteBuffer( CmdQueue, dY, CL_FALSE, 0, DATASIZE, hY, 0, NULL, NULL );
	if( status != CL_SUCCESS )
...
#else
	fprintf( stdout, "Array Size: %6d , Work Elements: %4d , MegaPointsProcessedPerSecond: %10.2lf\n",
