...
def toyProblem():
  ...
  for k in range(150):
    _, _, SSE = kMeansClustering(X, k=k+1, max_iters=max_iters, visualize=False)
...
def initalizeCentroids(dataset, k):
  ...
  n = len(dataset)
  return dataset[rng.integers(0, n, k)]
...
def computeAssignments(dataset, centroids):
  eucli_dist = [np.linalg.norm(dataset - i, axis=1) for i in centroids]
  # print(f'ASSIGMNETS: dataset.shape: {dataset.shape}')
  # print(f'ASSIGMNETS: centroids.shape: {centroids.shape}')
  # print(f'ASSIGMNETS: eucli_dist.shape: {eucli_dist.shape}')
  # print(f'ASSIGMNETS: np.argpartition(np.array(eucli_dist).T, 0).T[0]: {np.argpartition(np.array(eucli_dist).T, 0).T[0].shape}')
  return np.argpartition(np.array(eucli_dist).T, 0).T[0]
...
def updateCentroids(dataset, centroids, assignments):
  ...
  for i, point in enumerate(dataset):
    target_centroid = assignments[i]
    centroids[target_centroid] += point
    counts[target_centroid] += 1
  
  # print(f'centroids.shape: {centroids.shape}')
  # print(f'type(centroids): {type(centroids)}')
...
def calculateSSE(dataset, centroids, assignments):
  return np.sum(np.linalg.norm(dataset - centroids[assignments], axis=1))
