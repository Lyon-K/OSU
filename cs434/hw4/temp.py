...
def get_nearest_neighbors(example_set, query, k):
    # Use broadcasted subtraction and batched norm calculations to compute distances
    eucli_dist = np.linalg.norm(example_set - query, axis=1)
    print(f'example_set.shape: {example_set.shape}')
    print(f'query.shape: {query.shape}')
    exit()
    # Find and return the indexes of the k closest points
    idx_of_nearest = np.argpartition(eucli_dist, k-1)[:k]
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    # Can't have more neighbors than the number of examples we have
    k = min(k, len(examples_X))

    # Find the indices fo the k nearest neighbors
    neighbors = get_nearest_neighbors(examples_X, query, k)

    # Use the neighbors to predict a label for this query point
    neighbor_labels = [examples_y[row,0] for row in neighbors]
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
  ...
  # Split the dataset into equally sized chunks
  folds_X = np.split(train_X, num_folds)
  folds_y = np.split(train_y, num_folds)

  for i in range(num_folds):

    # Gather all folds except i and restack them to a matrix for our predict function
    train_X_without_i = np.vstack([folds_X[j] for j in range(num_folds) if j != i])
    train_y_without_i = np.vstack([folds_y[j] for j in range(num_folds) if j != i])

    # Grab the held-out fold's features and labels
    val_X_i = folds_X[i]
    val_y_i = folds_y[i]

    # Predict the labels for the held out fold using the remaining folds
    pred_labels_i = predict(train_X_without_i, train_y_without_i, val_X_i, k)

    # Compute accuracy of this fold and store it
    acc = compute_accuracy(val_y_i, pred_labels_i)
    val_acc.append(acc)

  # return the mean and variance values
  return np.mean(val_acc), np.var(val_acc)

# For those interested in list comprehensions, this version uses those only
def cross_validation_list_comp(train_X, train_y, num_folds=4, k=1):
  s_X, s_y = np.split(train_X, num_folds), np.split(train_y, num_folds)
  acc = [ compute_accuracy(s_y[i], predict(np.vstack([s for j,s in enumerate(s_X) if j!=i]), 
                                           np.vstack([s for j,s in enumerate(s_y) if j!=i]), 
                                           s_X[i],k)) for i in range(num_folds)]
